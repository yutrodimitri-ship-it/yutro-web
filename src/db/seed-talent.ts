...
async function seed() {
  ...
  await db
    ...
    .values({
      ...
      market: p.market,
      rightsDurationEs: p.rightsDuration.es,
      rightsDurationEn: p.rightsDuration.en,
      exclusivityMode: p.exclusivityMode,
      exclusivityCategoryEs: p.exclusivityCategory?.es ?? null,
      exclusivityCategoryEn: p.exclusivityCategory?.en ?? null,
      exclusivityHelpEs: p.exclusivityHelp.es,
      exclusivityHelpEn: p.exclusivityHelp.en,
      maxTalents: p.maxTalents,
      maxExclusive: p.maxExclusive,
      startDate: p.startDate,
      blockedTalentCodes: p.blockedTalentCodes,
      status: p.status,
    })
    .onConflictDoNothing();
