 ...
 *   - Landings SEO de servicios: /produccion/[slug]
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // ── Landings SEO de servicios: /produccion/[slug] ───────────
  // (Heredadas de /servicios/[slug]; el redirect 301 de next.config
  //  preserva las URLs antiguas.)
  const landingEntries = locales.flatMap((locale) =>
    serviceLandings.map((landing) => ({
      url: `${SITE_URL}/${locale}/produccion/${landing.slug}`,
      lastModified: now,
      changeFrequency: "monthly" as const,
      priority: 0.9,
      alternates: withAlternates(`/produccion/${landing.slug}`),
    }))
  );
